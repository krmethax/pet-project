...
export default function MemberSignup() {
  ...
  const handleSignup = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("http://192.168.1.10:5000/api/auth/register", {
        method: "POST",
  ...
  const handleVerifyOTP = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("http://192.168.1.10:5000/api/auth/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      const response = await fetch("http://192.168.1.10:5000/api/auth/update-profile", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    try {
      ...
      if (!result.canceled) {
        ...
        if (key === "profile_image") {
          endpoint = "http://192.168.1.10:5000/api/member/upload-profile-image";
        } else {
          setLoading(false);
  ...
  const handleCancelRegistration = async () => {
    setLoading(true);
    try {
      const response = await fetch("http://192.168.1.10:5000/api/auth/delete-registration", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
