...
export default function PetTypes() {
  ...
  const fetchPetTypes = useCallback(() => {
    setLoading(true);
    axios
      .get("http://192.168.133.111:5000/api/admin/pet-types")
      .then((response) => {
    ...
    try {
      if (modalMode === "add") {
        const response = await axios.post(
          "http://192.168.133.111:5000/api/admin/pet-types",
          {
      ...
      if (error.response) {
        ...
        swal("Error", "Failed to save pet type", "error");
      }
    }
  };
    ...
    if (confirm("Are you sure you want to delete this pet type?")) {
      try {
        const response = await axios.delete(
          `http://192.168.133.111:5000/api/admin/pet-types/${pet_type_id}`
        );
        swal("Success", response.data.message, "success");
        fetchPetTypes();
      } catch (error) {
