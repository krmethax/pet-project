...
export default function Service() {
  ...
  const fetchServiceTypes = useCallback(() => {
    setLoading(true);
    axios
      .get("http://192.168.133.111:5000/api/admin/service-types")
      .then((response) => {
    ...
    try {
      if (modalMode === "add") {
        const response = await axios.post(
          "http://192.168.133.111:5000/api/admin/service-types",
          {
    ...
    if (confirm("Are you sure you want to delete this service type?")) {
      try {
        const response = await axios.delete(
          `http://192.168.133.111:5000/api/admin/service-types/${service_type_id}`
        );
        swal("Success", response.data.message, "success");
        fetchServiceTypes();
      } catch (error) {
